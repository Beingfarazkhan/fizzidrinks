...
const Scene = ({ sentence, flavor }: SkyDiveProps) => {
    ...
    useGSAP(() => {
        if (!cloudsRef.current || !wordsRef.current || !canRef.current || !cloud1Ref.current || !cloud2Ref.current) return
        ...
        gsap.set([cloud1Ref.current.position, cloud2Ref.current.position], { ...getXYPosition(DISTANCE) })
    ...
    return (
        <group ref={groupRef} >
            <group rotation={[0, 0, 0.5]}>
                ...
                <FloatingCan flavor={flavor} ref={canRef} floatIntensity={3} floatSpeed={3} rotationIntensity={0}>
